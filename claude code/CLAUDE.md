...
**Universal repository-level rules for Claude Code Cloud sessions and coding agents.**

Use this file as the durable repository contract. Manual prompts pasted into chat may narrow the current goal, but should not repeat this file unless they need a stricter task-specific rule.

Use only this family of files as persistent repository guidance for Claude Code Cloud sessions: this root file plus applicable nested `CLAUDE.md` files. Do not depend on generic or other-agent memory files for instruction precedence.
...
3. Local `CLAUDE.md` rules in affected folders, if the repository uses them.
...
- Work in the current Claude Code Cloud session. Do not assume a hidden background worker or extra delegation mechanism unless the user explicitly provides one.
...
- If the requested outcome cannot be fully completed in the Claude Code Cloud worktree, complete every safe reachable part and report the concrete blocker.

## Repository grounding

- Treat the Claude Code Cloud worktree as the editable source of truth.
...
- For audit findings, implementation briefs, validation verdicts, navigation maps, evolution plans, and repo-derived documentation, cite stable anchors as `path:line[-line]` plus the nearest symbol when possible.
