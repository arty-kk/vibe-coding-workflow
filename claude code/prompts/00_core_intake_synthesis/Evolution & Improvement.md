...
- Analysis/documentation mode: inspect the repo and create or update only `evolution_plan.md` at the repository root when file writes are available.
...
Keep task specs implementation-ready, but these are roadmap specs, not chat work item output. If a future step needs a focused manual prompt, use `Synthesis.md` or `Task Shaping.md`.
