...
Create focused work items only for concrete repo-supported findings. Each finding must include evidence anchors as `path:line[-line]`, owner boundary, affected roles/scopes, impact, and validation path. Do not include generic security advice without repository evidence.
