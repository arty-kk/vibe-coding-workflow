...
Create focused task-subs only for concrete repo-supported findings. Each finding must include `path:line[-line]` evidence, owner layer, affected records/states, impact, and validation path. Do not suggest broad database redesign without a reachable issue.
