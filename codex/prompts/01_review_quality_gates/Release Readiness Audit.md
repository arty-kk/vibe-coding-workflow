...
- Every task-sub must cite `path:line[-line]` evidence for the release risk and affected owner.
...
Produce only native Codex Cloud task-subs grouped by P0/P1/P2, with a one-line release verdict: `ship`, `ship with known P2s`, `hold for P1/P0`, or `insufficient evidence`. If none qualify, output each priority with `None`.
