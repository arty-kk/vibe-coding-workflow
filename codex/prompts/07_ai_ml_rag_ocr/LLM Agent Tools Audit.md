...
Create focused task-subs only for concrete repo-supported findings. Each finding must include evidence anchors as `path:line[-line]`, affected tool/agent flow, safety/correctness impact, and validation/eval path. Do not include generic AI safety advice without repository evidence.
