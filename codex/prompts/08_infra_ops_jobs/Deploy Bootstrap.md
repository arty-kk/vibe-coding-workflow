...
- Execute mode: create a minimal production-usable deployment setup when repo evidence supports it.
...
Run relevant repo-native checks plus workflow syntax sanity where possible, Docker build/config checks when Docker is present or created, shell syntax checks for scripts, and config rendering sanity for Nginx. Document checks that cannot run in Codex Cloud.
