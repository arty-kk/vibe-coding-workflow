...
- Execute mode: change the repository when a focused instrumentation improvement is justified by current evidence.
...
- If the user named a flow, event, metric, log gap, dashboard contract, or analytics audit task-sub, improve that owner and directly affected calls.
